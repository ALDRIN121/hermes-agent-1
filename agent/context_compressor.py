    ...
    def _build_chunk_digests(self, turns: List[Dict[str, Any]]) -> str:
        ...
        for ci in range(n_chunks):
            ...
            try:
                ...
                # A digest cut off by the output cap is partial and reads as
                # complete once embedded in the summary — degrade to the
                # recover-via-session_search placeholder instead (pi#7048).
                if _response_finish_reason(resp) == "length":
                    raise RuntimeError(
                        "lean chunk digest hit the output token cap "
                        "(finish_reason=length); discarding partial digest"
                    )
                from agent.agent_runtime_helpers import strip_think_blocks
