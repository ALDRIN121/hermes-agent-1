...
export const ja = defineLocale({
  ...
  settings: {
    ...
    vault: {
      title: '資格情報ボールト',
      blurb:
        'エージェントが値を一切見ることなくサイトへサインインするために使える、暗号化されたローカル資格情報です。モデルに表示されるのはラベルとオリジンだけです。',
      count: n => `${n} 件保存済み`,
      loadFailed: 'ボールト項目を読み込めませんでした',
      empty: '保存された資格情報はまだありません',
      emptyDesc:
        'ログインを追加すると、エージェントがそのサイトに代わりにサインインできます。エージェントが受け取るのは不透明なハンドルだけで、パスワードは決して渡りません。',
      add: '資格情報を追加',
      addTitle: '資格情報を追加',
      addDescription: 'このマシン上に暗号化して保存されます。エージェントが秘密の値を見ることはありません。',
