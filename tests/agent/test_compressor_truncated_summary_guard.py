...
Covers all three compressor summarization sites:
  ...
  2. ``_micro_summarize_one`` (micro-compact rolling summary) — length stop
     ...
     unabsorbed.
  3. ``_build_chunk_digests`` (lean chunk digests) — length stop degrades the
     chunk to the recover-via-session_search placeholder.
"""
...
class TestChunkDigestTruncationGuard:
    def test_length_stop_degrades_to_placeholder(self):
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True)
        turns = [
            {"role": "assistant", "content": "did a thing " * 50},
            {"role": "tool", "content": "tool output " * 50, "tool_call_id": "t1"},
        ]
        with patch(
            "agent.auxiliary_client.call_llm",
            return_value=_mock_response("partial diges", "length"),
        ):
            out = c._build_chunk_digests(turns)
        assert "digest unavailable" in out
        assert "partial diges" not in out

    def test_stop_finish_reason_keeps_digest(self):
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True)
        turns = [
            {"role": "assistant", "content": "did a thing " * 50},
        ...
        with patch(
            "agent.auxiliary_client.call_llm",
            return_value=_mock_response("complete digest", "stop"),
        ):
            out = c._build_chunk_digests(turns)
        assert "complete digest" in out
        assert "digest unavailable" not in out
